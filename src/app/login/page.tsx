...
import { authClient, isMembroBI } from "@/lib/auth-client";

// ─── Personalizar aqui ─────────────────────────────────────────
const BRAND = {
  prefix: "Meu",
  suffix: "BI",
  tagline: "Inteligência de Negócios",
  hero: "Inteligência de negócios em tempo real",
};
// ───────────────────────────────────────────────────────────────
...
export default function LoginPage() {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    const ok = await isMembroBI();
    if (!ok) {
      setError("Usuário sem acesso ao BI. Contate o administrador.");
      ...
      >
        ...
        <div className="relative">
          ...
          <div className="mt-12 flex gap-8">
            <Stat value="Dashboards" label="Visões consolidadas" />
            <Divider />
            <Stat value="Métricas" label="KPIs operacionais" />
            <Divider />
            <Stat value="Real" suffix="time" label="Atualização contínua" />
          </div>
        </div>
