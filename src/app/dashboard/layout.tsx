...
// ─── Personalizar aqui ─────────────────────────────────────────
const BRAND = { prefix: "Meu", suffix: "BI", tagline: "Inteligência de Negócios" };

const navOperacional = [
  { href: "/dashboard", label: "Visão Geral", icon: LayoutDashboard },
  { href: "/dashboard/vendas", label: "Vendas", icon: TrendingUp },
  { href: "/dashboard/operacao", label: "Operação", icon: BarChart3 },
  ...
  return (
    <div className="app-shell">
      <aside className={`sidebar${collapsed ? " collapsed" : ""}`}>
        ...
        <nav className="sidebar-nav" style={{ padding: "8px 12px", flex: 1, display: "flex", flexDirection: "column", gap: 2 }}>
        ...
        <div className="sidebar-footer" style={{ borderTop: "1px solid var(--border)", padding: 10 }}>
          {!collapsed ? (
            <div style={{ display: "flex", alignItems: "center", gap: 10, padding: "8px 8px", borderRadius: "var(--radius)" }}>
              <div
                style={{
              ...
              <div style={{ flex: 1, overflow: "hidden", lineHeight: 1.25 }}>
                ...
                <div style={{ fontSize: "0.62rem", color: "var(--text-muted)", textTransform: "uppercase", letterSpacing: "0.06em", fontWeight: 600 }}>
                  Usuário
                </div>
              </div>
              <button
                ...
                style={{ background: "transparent", border: "none", cursor: "pointer", padding: 6, color: "var(--text-muted)", display: "flex", borderRadius: 6 }}
              >
            ...
            <div style={{ display: "flex", flexDirection: "column", alignItems: "center", gap: 10 }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--accent)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.75rem" }}>
                {initials}
              </div>
              <button onClick={handleSignOut} title="Sair" style={{ background: "transparent", border: "none", cursor: "pointer", color: "var(--text-muted)", padding: 6, display: "flex" }}>
                <LogOut size={16} />
              </button>
            </div>
          )}
      ...
      <main className={`main-content${collapsed ? " collapsed" : ""}`}>
        <header className="page-header">
          <div>
            ...
            <p className="page-subtitle">Visão geral — Hoje</p>
          </div>
          <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
            ...
            <button className="btn-icon btn-ghost" title="Notificações" style={{ position: "relative" }}>
              <Bell size={16} />
              <span style={{ position: "absolute", top: 2, right: 2, minWidth: 16, height: 16, borderRadius: 999, background: "var(--danger)", color: "#fff", fontSize: "0.6rem", fontWeight: 700, display: "flex", alignItems: "center", justifyContent: "center", padding: "0 4px" }}>
                0
              </span>
            </button>
            <div style={{ display: "flex", alignItems: "center", gap: 10, cursor: "pointer" }}>
              <div style={{ width: 32, height: 32, borderRadius: "50%", background: "var(--accent)", color: "#fff", display: "flex", alignItems: "center", justifyContent: "center", fontWeight: 700, fontSize: "0.78rem" }}>
                {initials}
              </div>
